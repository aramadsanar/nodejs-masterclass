    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            ...
            if (alertWarranted) {
                ...
                console.log('check outcome not changed yet. no update needed!')
            }
        } else {
            console.log("Cannot update check!")
        }
    })
}
...
workers.alertUserToStatusChange = (newCheckData) => {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            console.log("Success: User was alerted via sms on status change!")
        } else {
            console.log("Error cannot send sms alert to user regarding check status change!")
        }
    })
}
...
workers.rotateLogs = () => {
    ...
    _logs.list(false, (err, logs) => {
        if (!err && logs && logs.length > 0) {
            ...
            console.log("Error cannot fild any logs to rotate!")
        }
    })
}
